...
export function AgreementStep({ onNext, onBack }: Props) {
  const [agreed, setAgreed] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);

  const settingsQuery = useQuery({
    queryKey: ["system_settings", "delivery_partner_agreement_md"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("system_settings")
        .select("delivery_partner_agreement_md")
        .eq("id", 1)
        .single();
      if (error) throw error;
      return data.delivery_partner_agreement_md;
    },
  });

  async function onSubmit() {
    if (!agreed) {
      setFormError("You must accept the Cotto Delivery Partner Agreement to continue.");
      return;
    }
    setSubmitting(true);
    setFormError(null);
  ...
  return (
    <View className="gap-4">
      <Text className="text-2xl font-bold text-white">Cotto Delivery Partner Agreement</Text>
      ...
      <Pressable className="flex-row items-center gap-3" onPress={() => setAgreed(!agreed)}>
        ...
        <Text className="flex-1 text-white/80">I accept the Cotto Delivery Partner Agreement.</Text>
      </Pressable>
