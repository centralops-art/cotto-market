-- 0055: allow a vendor to see and claim their own ready delivery orders.
-- Illinois cottage food law requires cottage food operators to deliver their
-- own products themselves -- the Phase 8 "self-claim block" (0010, 0036,
-- 0043) was built on the opposite assumption (vendors only deliver for each
-- other) and is now a legal-compliance gap, not just a product choice.
-- Founder decision (2026-08-15): keep everything else about the claim flow
-- identical (same pool query, same claim_delivery RPC, same payout split) --
-- a vendor's own ready delivery orders now simply appear in their own pool
-- like anyone else's and are claimed the same way.
...
begin
  ...
  -- Self-claim block removed 2026-08-15 (migration 0055): Illinois cottage
  -- food law requires vendors to be able to deliver their own orders.
